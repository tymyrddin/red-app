...
An attacker will need to log in to `wiener:peter`; and then log in to Carlos's account.
...
9. Copy the session cookie from the Set-Cookie header in the displayed response and add it to your request.
10. Right-click on your request and select "Change request method". Burp will convert it to a POST request.
11. Send the request to delete Carlos and solve the lab.
