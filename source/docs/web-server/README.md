# Introduction

## What?

![RootMe](../../_static/images/rootme.png)

## Why?

The fast, easy, and **very** affordable way to test hacking skills.

## How?

* [Insecure Code Management](git.md)
* [Directory traversal](traversal.md)
* [File upload: null byte](upload-null.md)
* [PHP assert()](php1.md)
* [PHP Filters](php2.md)
* [PHP Register globals](php3.md)
* [JWT Introduction](jwt1.md)
* [JWT (not) revoked token](jwt2.md)
* [JWT weak secret](jwt3.md)
* [Command injection: filter bypass](ci-filter-bypass.md)
* [Java: Server-side Template Injection (SSTI)](java-ssti.md)
* [Local file inclusion](lfi.md)
* [Local file inclusion: double encoding](lfi-double.md)
* [PHP preg_replace](php-preg-replace.md)
* [PHP type juggling](php-type-juggling.md)
* [SQL injection: authentication](sql-authentication.md)
* [SQL injection: string](sql-string.md)
* [PHP path truncation](php-truncation.md)
* [PHP serialisation](php-serialisation.md)
* [SQL injection: numeric](sql-numeric.md)
* [SQL injection: routed](sql-routed.md)
* [SQL truncation](sql-truncation.md)
* [XPath injection: authentication](xpath-inject-auth.md)
* [SQL injection: time-based](sql-time.md)
